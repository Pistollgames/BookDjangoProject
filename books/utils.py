...
def update_user_genre_preferences(user, book, added=True):
    if added:
        for genre in book.genres.all():
            ...
            if not created:
                pref.book_count += 1
                pref.total_pages += book.pages or 0
                pref.save()
    else:
        for genre in book.genres.all():
            try:
...
def calculate_user_genre_stats(user):
    ...
    for pref in preferences:
        ...
        stats.append({
            ...
            'percentage': round((pref.book_count / Book.objects.filter(user=user).count()) * 100, 1)
            if Book.objects.filter(user=user).count() > 0 else 0
        })
